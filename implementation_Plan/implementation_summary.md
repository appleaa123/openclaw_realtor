...
| What | Where you configure it | Persisted how |
|---|---|---|
| API keys & secrets | Render dashboard → Environment | Render env vars |
| AI personality, rules, skills | This repo → `workspace-templates/` + `skills/` | Render disk (copied on first boot) |
| WhatsApp channel for **team** to chat with AI | OpenClaw Control UI (your Render URL) | Render disk `/data` |
| Email channel for **inbound** tenant emails | OpenClaw Control UI | Render disk `/data` |
| WhatsApp outbound to **tenants/vendors** (wacli) | Render shell — one-time `wacli auth` | Render disk `/data/.wacli` |
| Email outbound to tenants (himalaya) | Render shell — one-time config file | Render disk `/data/.config/himalaya` |
| Cron jobs | HEARTBEAT.md (schedule defined) + OpenClaw Control UI to activate | Render disk |
...
| Key | Value | Notes |
|---|---|---|
| `SETUP_PASSWORD` | any strong password | First-time UI login |
| `OPENAI_API_KEY` | `sk-...` | For AI responses |
| `GEMINI_API_KEY` | `AIza...` | Optional, for vision tasks |
| `SUPABASE_URL` | `https://xxxx.supabase.co` | Your Supabase project URL |
| `SUPABASE_SERVICE_KEY` | `eyJ...` | Supabase service role key (not anon) |
| `HIMALAYA_CONFIG` | `/data/.config/himalaya/config.toml` | Persists himalaya config to disk |

`OPENCLAW_GATEWAY_TOKEN` is auto-generated by Render — leave it alone. After setting these, trigger a **Manual Deploy**.

### Step 2 — Open the OpenClaw Control UI

Open your Render service URL in a browser, e.g. `https://openclaw-xxxx.onrender.com`, and log in with `SETUP_PASSWORD`. This is where the team chats with the AI.

### Step 3 — Connect WhatsApp (team channel)

Done entirely in the OpenClaw Control UI:

1. **Channels** → **Add Channel** → **WhatsApp**
2. Scan the QR code with the **AI agent phone** (a dedicated number — not a team member's personal phone)
3. Once connected, team members can WhatsApp that number to talk to the AI
4. Set `dmPolicy: "allowlist"` in `openclaw.json` and add all 9 team member E.164 numbers to `allowFrom`

### Step 4 — Connect email (inbound tenant emails)

Done in the OpenClaw Control UI:

1. **Channels** → **Add Channel** → **Email**
2. Enter shared team inbox IMAP credentials (host, port, login, app password)
3. OpenClaw will now receive tenant emails and route them to the AI

This handles inbound only. Outbound sending (himalaya) is configured in Step 6.

### Step 5 — Configure wacli (outbound WhatsApp to tenants/vendors)

One-time interactive QR scan. Use Render's **Shell** tab:

```sh
export WACLI_STORE=/data/.wacli
wacli auth                        # scan QR with the AI agent phone
wacli chats list --limit 5        # verify
```

Add to Render env vars to persist across redeploys:
```
WACLI_STORE=/data/.wacli
```

### Step 6 — Configure himalaya (outbound email to tenants)

In Render **Shell**:

```sh
mkdir -p /data/.config/himalaya

cat > /data/.config/himalaya/config.toml << 'EOF'
[accounts.team]
email = "yourteam@yourdomain.com"
display-name = "Your Team Name"
default = true

backend.type = "imap"
backend.host = "imap.yourdomain.com"
backend.port = 993
backend.encryption.type = "tls"
backend.login = "yourteam@yourdomain.com"
backend.auth.type = "password"
backend.auth.raw = "YOUR_APP_PASSWORD"

message.send.backend.type = "smtp"
message.send.backend.host = "smtp.yourdomain.com"
message.send.backend.port = 587
message.send.backend.encryption.type = "start-tls"
message.send.backend.login = "yourteam@yourdomain.com"
message.send.backend.auth.type = "password"
message.send.backend.auth.raw = "YOUR_APP_PASSWORD"
EOF

himalaya envelope list   # verify
```

Gmail: IMAP `imap.gmail.com:993`, SMTP `smtp.gmail.com:587`. Use a Gmail App Password (not your account password).

### Step 7 — Set manager WhatsApp number in SOUL.md

Edit `workspace-templates/SOUL.md` line 10 — replace `+1XXXXXXXXXX` with the manager's real E.164 number. Push to git and redeploy. Then run in Render Shell to apply immediately without waiting for next boot:
...
```sh
# Daily client care route + email digest (7am)
openclaw cron add \
  --schedule "0 7 * * *" \
  --session isolated \
  --model opus \
  --announce \
  "Run client-care-route skill and compile overnight team inbox emails into digest. Send formatted WhatsApp summary to team group."
...
|---|---|
| Type "hello" in Control UI | AI responds |
| WhatsApp the AI agent number | AI responds |
| Send email to team inbox → ask "any new emails?" | AI surfaces it |
| Ask AI to draft a tenant email → approve | Email sent via himalaya, appears in Sent folder |
| Ask AI to message a vendor → approve | Vendor receives WhatsApp via wacli |
| Say "escalate [tenant name]" | WhatsApp DM goes to manager's number (not group) |
| Say "run client-care-route now" | AI executes and reports route |
