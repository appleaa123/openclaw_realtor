...
### **Use Case 0: Team management & Daily Manager Communication**

**Problem:** Manager gets real time updates on all other use cases and communicate with AI in real time for all use cases and with all access.

_Trigger:_ This use case does not get triggered automatically. It acts as a communication chatbot with the realtor to see summaries from other use cases, answer questions, and act accordingly to the realtor's demands and asks.

**Functional Requirements:**

- **Summarization:** the team management agent will get summarizations from all other use cases every 12 hours. Starting from 6am Eastern Time, then 12pm, 6pm, 12am each day. Tell the realtor what is going on for each use cases such as rent, property visits, fixing and maintenance, and legal actions.
- **Feedback & Actions:** The realtor will talk to the chatbot about anything and the AI agent will get feedback, and take action accordingly, with the human realtor's approval.
- **System Access:** The realtor will have access to all data, databases, and have full access of any adjustments for the entire system.
- **Request Triage:** Based on the realtor's question, this AI agent will either access the database to pull the information, adjust any data from the database. If the realtor asks questions regarding other use cases, first send the case updates from the nearest previous summarization cycle, if the realtor requires real time information, then pull the most updated information with human realtor's approval. For example, if the realtor asks if the dishwasher is fixed for a specific property at 9am, the agent will pull the fix status from 6am summarization cycle. If the realtor asks about real time update, the agent will pull from the latest information from the database with the timestamp.
...
**Trigger:** Cron job detects a lease is 4 months away from its 12-month anniversary from tenant table, last_rent_adjustment_date column.
...
- **Core Fields:**
  ...
  - property_id: a random ID the system assigned to this property when it’s entered.
  - landlord_name: the name of the landlord of the property. Example: Joe Doe
  ...
  - status: the current status of the property. includes:
    ...
    - late_payment: the rent payment is late from the tenant or problems with rent payments. This status gets triggered by Use Case 3\.
    - fix_needed: the property requires fix. This status gets triggered by Use Case 2\.
  ...
  - time_stamp: time stamp of any changes on this table. Format: dd/mm/yyyy
...
- **Core Fields:**
  ...
  - tenant_id: a random ID the system assigned to this tenant when it’s entered.
  - phone: phone number of the tenant. Example: 4161231234
  - email: email of the tenant. Example test@gmail.com
  - lease_start: the start date of the lease. Format: dd/mm/yyyy
  - rent_amount: the amount of the rent per month. Example: 1500
  - birthday: the birthday of the tenant. Format: dd/mm/yyyy
  - last_time_rent_adjustment_date: the date of last rent adjustment. Format: dd/mm/yyyy
...
- **Core Fields:**
  ...
  - tenant_id: a random ID the system assigned to this tenant when it’s entered. Must be consistent with the tenant_id in the Tenant table.
...
- **Core Fields:**
  - property_id: a random ID the system assigned to this property when it’s entered. Must be the same as the
  - property_address: property address without postal code. Example: 44 Addison St.
  - issue_type: the type of the issue by category. Values are only from the following options: Improper Surface Grading, water damage, electrical damage, appliance, roof, HVAC, and maintenance.
  ...
  - assigned_worker: the assigned business for the issue. For example: abc HVAC System Fixing Co.
- **Table 5:** vendors
- **AI Purpose:** Tracks the current vendors working with the realtor, their expertise, and use as vendor recommendation.
- **Core Fields:**
  - vendor_id: a random ID the system assigned to each vendor.
  - name: name of the vendor. Example: Joe Doe.
  - trade_type: type of service the vendor provides.
  - phone: phone number of the vendor. Example: 4161231234
  - email: email of the vendor. Example test@gmail.com
  - city: city of the vendor is located in.
  - notes: some notes regarding the vendor's availability, service time, and working style
  - timestamp: time stamp of the vendor start working with the realtor.
...
- **Headless Server Configuration:** Use the openclaw-mcp integration to bridge the internal OpenClaw Gateway to external agents. This requires configuring environment variables such as OPENCLAW_BASE_URL (local Gateway URL), OPENCLAW_GATEWAY_TOKEN (bearer token for auth), and OPENCLAW_AGENT_ID.
...
- **Blueprint Configuration (render.yaml):**
  ...
  - Configure the OPENCLAW_STATE_DIR to /data/.openclaw to persist credentials.
  - Configure the OPENCLAW_WORKSPACE_DIR to /data/workspace to save generated PDFs and custom skills.
...
- **Execution Flow:** The AI extracts property and tenant data from Supabase and passes it to the Python script. The script uses PdfReader.get_form_text_fields() to map data to the exact programmatic text boxes on official, blank N4/N9/N12 templates.
...
- **The Tenant ID Identifier:** Every table must include a tenant_id column representing the SaaS client (brokerage) to enable shared-database multi-tenancy.
- **Row Level Security (RLS):**
  - Enable RLS on all tables to ensure cryptographic data isolation.
  - Inject the specific tenant_id into the user's app_metadata via a Supabase Auth Hook upon account creation.
  - Create policies that append implicit WHERE clauses to every query (e.g., USING (tenant_id \= (auth.jwt() \-\> 'app_metadata' \-\>\> 'tenant_id')::uuid)).
- **Indexing:** Create a composite index on (tenant_id, property_id) across all tables to prevent catastrophic performance degradation during RLS filtering.
